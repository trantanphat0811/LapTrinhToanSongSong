...
# 2. Counting Sort và Counting Sort song song
def counting_sort(arr, max_value):
    """Thuật toán Counting Sort tiêu chuẩn."""
    count = [0] * (max_value + 1)
    output = []
...
def threaded_counting_sort(arr, max_value):
    """Counting Sort có sử dụng đa luồng."""
    if len(arr) <= 100_000:  # Nếu mảng nhỏ, dùng Counting Sort trực tiếp
        return counting_sort(arr, max_value)

    mid = len(arr) // 2
    left_sorted = []
    right_sorted = []

    t1 = threading.Thread(target=lambda: left_sorted.extend(counting_sort(arr[:mid], max_value)))
    t2 = threading.Thread(target=lambda: right_sorted.extend(counting_sort(arr[mid:], max_value)))
...
def sort_numbers_from_file(input_file="numbers.txt", output_file="sorted_numbers.txt", max_value=25_000_000):
    ...
    for chunk in read_numbers_in_chunks(input_file):
        thread = threading.Thread(target=lambda c: sorted_chunks.append(threaded_counting_sort(c, max_value)), args=(chunk,))
